function scr_veronica_attacks(){
	...
	attack_veronica_main_01 = function() {
		...
		if count mod 45 == 0 {
			...
			repeat(global.main_stats.difficulty + 1) {
				for(var i = -2; i < 3; i++) {
					var shotSpeed = speedT - (abs(i) * 0.1)
					shot_CreateShotA1(x,y,shotSpeed,baseAngle + (8 * i),spr_danmaku_bullet,10,15)
					speedT -= 0.25;
				}
			}
		}
	...
	attack_veronica_main_02 = function() {
		...
		if count mod 6 == 0 and speed > 0.5 {
			sound_ObjSound_Play(se_tan01)
	...
	attack_veronica_main_03 = function() {
		...
		if count mod 20 == 0 {
			...
			repeat(5) {
				shot_CreateShotA1(leftWallX,8,1,275,spr_danmaku_bullet,10,15);
				shot_CreateShotA1(rightWallX,8,1,265,spr_danmaku_bullet,10,15);
			}
		}
		if count mod 50 == 0 {
			sound_ObjSound_Play(se_kira01)
	...
	attack_veronica_main_05 = function() {
		...
		if count mod 240 == 0 {
			...
			repeat(5) {
				shot_CreateStraightLaserA1(Xstart,0,270,28.8,1,180,spr_danmaku_laser,120,0);
				shot_CreateStraightLaserA1(0,Ystart,270,28.8,1,180,spr_danmaku_laser,120,0);
				Xstart += 56;
				Ystart += 56;
			}
		}
		if count mod 16 == 0 {
			...
			repeat(global.main_stats.difficulty + 1) {
				for(var i = -1; i < 2; i++) {
					shot_CreateShotA1(x,y,speedT,baseAngle + (i * 6),spr_danmaku_bullet,8,15);
				}
				speedT -= 0.25;
			}
		}
	...
	spell_card_veronica_01 = function() {
		...
		if count mod 90 == 0 and count > 0 {
			...
			repeat(2) {
				var spinrapier;
				if k == 0 { spinrapier = 32 } else { spinrapier = -32 }
				var inst = shot_CreateShotA1(x_pos + spinrapier,y_pos + 8,0.75,300,spr_danmaku_ball_s1,0,15)
				with(inst) {
					set_function = function() {
	...
	spell_card_veronica_02 = function() {
		...
		if count mod 75 == 0 and count > 0 {
			...
			repeat(6 + (global.main_stats.difficulty * 2)) {
				shot_CreateLooseLaserA1(x,y,0.6,baseAngle,10,1,spr_danmaku_laser,10,10)
	...
	spell_card_veronica_03 = function() {
		...
		if count mod 3 == 0 and speed > 1 {
			for(var i = -1 - global.main_stats.difficulty; i < 2 + global.main_stats.difficulty; i++) {
				shot_CreateShotA1(x,y,0.25,(direction + 180) + ((i * 15) + irandom_range(-10,10)),spr_danmaku_bullet,5,15);
			}
		}
		if count mod 50 == 0 {
			repeat(4) {
	...
	spell_card_veronica_04 = function() {
		...
		if count >= 60 {
			if count mod 6 == 0 {
				...
				var inst = shot_CreateShotA1(x,y,1.75,baseAngle + offset,spr_danmaku_amulet,10,12);
				with(inst) {
					spawnFrame = other.count;
					set_function = function() {
						if count == 45 {
							if spawnFrame mod 12 == 0 {
								speed = 0.6;
							}
							else {
								speed = 2.4;
	...
	spell_card_veronica_06 = function() {
		...
		if count mod 140 == 0 and count > 0 {
			for(var i = -1; i < 2; i++) {
				var inst = shot_CreateLooseLaserA1(x,y,0,270 + 35 + (i * 20),4.8,1,spr_danmaku_laser,0,5);
				with(inst) {
					set_function = function() {
						if count == 40 {
	...
	spell_card_veronica_07 = function() {
		...
		if count >= 60 {
			...
			if count mod 12 == 0 {
				var clusterSize = irandom_range(3,6)
				repeat(clusterSize) {
					var inst = shot_CreateShotA1(choose(laserL.x_pos,laserR.x_pos),8,random_range(0.9,1.75),270 + irandom_range(-15,15),spr_danmaku_ball_m1,10,0)
					with(inst) {
						set_function = function() {
